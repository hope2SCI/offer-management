...
6. 通过数据洞察查看投递转化效果、渠道效果和城市投递分布。
7. 为后续 AI 简历建议、面试复盘和更深入投递分析预留入口。
...
- 核心指标卡片：总投递数、进入面试数、Offer 数、面试率、Offer 率、面试转 Offer 率；面试率和 Offer 率卡片需要展示计算公式说明。
- 求职漏斗：使用图表库展示总投递数、笔试数、面试数和 Offer 数的多色竖状条形图，并与渠道效果表同一行两列展示。
- 渠道效果表：渠道、投递数、面试数、Offer 数、面试率、Offer 率。
- 城市投递分布：按城市展示投递数量横向条形图。

统计口径：

- 转化率按岗位历史状态流转计算，不只看当前状态。
- 进入面试指历史上曾进入一面、二面、HR 面或 Offer。
- Offer 指历史上曾进入 Offer。
- 分母为历史上曾进入已投递及之后状态的岗位。
- 城市分布只统计岗位当前记录中的城市投递数量，不展示城市转化率。
...
- 本周投递数：本周一至本周日内新增的岗位记录数量，按岗位 `createdAt` 统计。
...
- 新增数据洞察页面。
- 实现核心转化指标卡片。
- 在面试率和 Offer 率卡片中展示计算公式。
- 使用图表库实现总投递、笔试、面试、Offer 的多色求职漏斗竖状条形图，并与渠道效果表同排展示。
- 实现渠道效果表。
- 实现城市投递数量条形图。
...
17. 用户可以在数据洞察页查看转化指标、渠道效果和城市投递分布。
18. 面试复盘入口存在，但不要求完整功能。
